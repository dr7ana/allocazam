...
namespace allocazam {
    ...
    namespace pool {
        ...
        class alignas(detail::cache_line_size) run_allocator {
            struct run_header {
          ...
          public:
            ...
            explicit run_allocator(size_t initial_bytes = 65536)
                    : _page_size{detail::detect_page_size()},
                      _next_growth{std::ranges::max(initial_bytes, size_t{65536})} {
                _add_owned_chunk(initial_bytes);
            }

            explicit run_allocator(std::span<std::byte> backing)
                    : _page_size{detail::detect_page_size()}, _next_growth{0} {
                _add_external_chunk(backing);
            }

            ~run_allocator() { _release_owned_chunks(); }

            run_allocator(const run_allocator&) = delete;
            run_allocator& operator=(const run_allocator&) = delete;
            run_allocator(run_allocator&&) = delete;
            run_allocator& operator=(run_allocator&&) = delete;
          ...
          private:
            ...
            static constexpr bool _flag(size_t size_and_flags, size_t bit) noexcept {
                return (size_and_flags & bit) != 0;
            }
            ...
            static constexpr bool _is_free(const run_header* h) noexcept {
                return _flag(h->size_and_flags, flag_is_free);
            }
            ...
            void _add_external_chunk(std::span<std::byte> backing) {
                if (backing.empty()) {
                    throw std::invalid_argument{"run_allocator backing must not be empty"};
                }

                void* aligned = backing.data();
                size_t space = backing.size();
                if (std::align(run_alignment, min_chunk_bytes, aligned, space) == nullptr) {
                    throw std::invalid_argument{"run_allocator backing alignment is insufficient"};
                }

                size_t chunk_bytes = space - (space % run_alignment);
                if (chunk_bytes < min_chunk_bytes) {
                    throw std::invalid_argument{"run_allocator backing too small"};
                }
