...
namespace {
    ...
    [[nodiscard]] constexpr size_t unordered_map_find_count(size_t count_per_iter) noexcept {
        return (count_per_iter + 3) / 4;
    }

    [[nodiscard]] constexpr size_t unordered_map_erase_count(size_t count_per_iter) noexcept {
        return count_per_iter / 2;
    }

    [[nodiscard]] constexpr size_t unordered_map_ops_per_iter(size_t count_per_iter) noexcept {
        return count_per_iter + unordered_map_find_count(count_per_iter) + unordered_map_erase_count(count_per_iter);
    }

    [[nodiscard]] double unordered_map_std_workload(size_t iterations, size_t count_per_iter, size_t batch_iterations) {
        using ns_t = std::chrono::nanoseconds;
        batch_iterations = at_least_one(batch_iterations);

        ns_t elapsed{};
        size_t total_ops = iterations * unordered_map_ops_per_iter(count_per_iter);

        for (size_t base = 0; base < iterations; base += batch_iterations) {
            size_t batch_count = std::min(batch_iterations, iterations - base);
            std::vector<std::unordered_map<int, int>> batch;
            batch.resize(batch_count);
            for (auto& values : batch) {
                values.reserve(count_per_iter);
            }
            ...
            for (size_t b : std::ranges::iota_view{size_t{0}, batch_count}) {
                ...
                size_t erase_count = unordered_map_erase_count(count_per_iter);
    ...
    [[nodiscard]] double unordered_map_allocazam_workload(
        ...
        using container_t = std::unordered_map<int, int, std::hash<int>, std::equal_to<int>, alloc_t>;
        using ns_t = std::chrono::nanoseconds;
...
int main(int argc, char** argv) {
    ...
    size_t iterations = parse_size_arg(argc, argv, 1, 5000);
    ...
    size_t ops_per_iter = unordered_map_ops_per_iter(count_per_iter);
    size_t ops = iterations * ops_per_iter;
    size_t warmup_iterations = std::ranges::max(iterations / warmup_divisor, size_t{1});

    std::cout << "unordered_map std allocator benchmark config\n";
    ...
    (void)unordered_map_std_workload(warmup_iterations, count_per_iter, batch_iterations);
    (void)unordered_map_allocazam_workload(warmup_iterations, count_per_iter, batch_iterations);
    run_stats stats = run_robust_benchmark(
            repeats,
            ops,
            [&] { return unordered_map_std_workload(iterations, count_per_iter, batch_iterations); },
            [&] { return unordered_map_allocazam_workload(iterations, count_per_iter, batch_iterations); });

    std::cout << "\nrobust benchmark summary\n";
    print_run_stats("unordered_map insert/find/erase workload", stats);
    ...
    std::cout << "\nrunning granular unordered_map matrix (" << sizes.size() << " sizes)\n";
    size_t idx = 0;
    for (size_t units : sizes) {
        ++idx;
        std::cout << "  unordered_map size " << units << " (" << idx << "/" << sizes.size() << ")\n" << std::flush;
        size_t granular_ops_per_iter = unordered_map_ops_per_iter(units);
        ...
        (void)unordered_map_std_workload(granular_warmup, units, batch_iterations);
        (void)unordered_map_allocazam_workload(granular_warmup, units, batch_iterations);
        run_stats granular_stats = run_robust_benchmark(
                repeats,
                granular_ops,
                [&] { return unordered_map_std_workload(granular_iterations, units, batch_iterations); },
                [&] { return unordered_map_allocazam_workload(granular_iterations, units, batch_iterations); });
    ...
    print_granular_table("granular unordered_map matrix", rows);
